...
<div align="center">
  ...
  [![Version](https://img.shields.io/badge/Version-5-red?style=flat-square)](https://github.com/DKhorov/Back-end-SpotiyClone)
...
| [🫂 Работа системы подписок ( UserConnect(r) )](#-защита-от-api-атак) | Реализованные меры безопасности |
...
<summary>🫂 Версия 5.0</summary>

- **👥 Возможность подписки на пользователей и отписка 
- **🥸 Просмотр подписок пользовтаеля 
- **⛓️ Добавлена логирование в checkAuth, чтобы увидеть, что именно происходит при проверке токена
- **📦 Базы данных PostgeSQL и SQLite временно убраны на mongoDB 
- **🎎 Запрос на получение всех пользователей с ролью 'артист'
- **🎛️ Убраны ошибки в работе control.js , userService.js

</details>
<details>

<summary>🚀 Версия 4.3.1</summary>

- **✨ Добавлен Eslint & Prettier для поддержания единого стиля при написании кода
- **✨ Весь основной код был перенесен в папку /src
- **✨ Создан конфиг nodemon.json
</details>
...
---
## 🫂 Работа системы подписок ( UserConnect(r) )
Система подписок позволяет пользователям подписываться на других пользователей, отписываться от них, а также просматривать списки подписчиков и подписок.

### Основные функции:
1. **Подписаться на пользователя** — пользователь может подписаться на другого пользователя.
2. **Отписаться от пользователя** — пользователь может отписаться от другого пользователя.
3. **Просмотр подписчиков** — пользователь может просмотреть список своих подписчиков.
4. **Просмотр подписок** — пользователь может просмотреть список пользователей, на которых он подписан.

---

### Модель данных

Каждый пользователь имеет два массива в своей модели:
- **`followers`** — массив ID пользователей, которые подписаны на текущего пользователя.
- **`following`** — массив ID пользователей, на которых подписан текущий пользователь.
...
- **Описание:** Позволяет текущему пользователю подписаться на другого пользователя.
- **Заголовок:** `Authorization: Bearer <ваш_токен>`
- **Параметры:**
  - `userId` — ID пользователя, на которого нужно подписаться.
- **Пример запроса:**
  ```bash
  POST /users/64f1a2b3c4d5e6f7a8b9c0d1/follow
...
- **Пример ответа:**
  ```json
  {
    ...
    "user": {
      ...
      "fullName": "John Doe",
      "followers": ["64f1a2b3c4d5e6f7a8b9c0d2"],
      "following": []
    }
  }
  ```
...
- **Описание:** Позволяет текущему пользователю отписаться от другого пользователя.
- **Заголовок:** `Authorization: Bearer <ваш_токен>`
- **Параметры:**
  - `userId` — ID пользователя, от которого нужно отписаться.
- **Пример запроса:**
  ```bash
  POST /users/64f1a2b3c4d5e6f7a8b9c0d1/unfollow
  Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
  ```
- **Пример ответа:**
  ```json
...
#### 3. **Просмотр подписчиков**
- **Метод:** `GET`
- **URL:** `/users/:userId/followers`
- **Описание:** Возвращает список подписчиков пользователя.
- **Заголовок:** `Authorization: Bearer <ваш_токен>`
- **Параметры:**
  - `userId` — ID пользователя, чьих подписчиков нужно получить.
- **Пример запроса:**
  ```bash
  GET /users/64f1a2b3c4d5e6f7a8b9c0d1/followers
  Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
  ```
- **Пример ответа:**
  ```json
...
- **Описание:** Возвращает список пользователей, на которых подписан текущий пользователь.
- **Заголовок:** `Authorization: Bearer <ваш_токен>`
- **Параметры:**
  - `userId` — ID пользователя, чьи подписки нужно получить.
- **Пример запроса:**
  ```bash
  GET /users/64f1a2b3c4d5e6f7a8b9c0d1/following
  Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
  ```
- **Пример ответа:**
  ```json
...
1. **Подписка:**
   - Когда пользователь отправляет запрос на подписку, система:
     - Находит пользователя, на которого подписываются, и пользователя, который подписывается.
     - Добавляет ID подписчика в массив `followers` пользователя, на которого подписываются.
     - Добавляет ID пользователя, на которого подписываются, в массив `following` подписчика.
     - Сохраняет изменения в базе данных.

2. **Отписка:**
   - Когда пользователь отправляет запрос на отписку, система:
     - Находит пользователя, от которого отписываются, и пользователя, который отписывается.
     - Удаляет ID подписчика из массива `followers` пользователя, от которого отписываются.
     - Удаляет ID пользователя, от которого отписываются, из массива `following` подписчика.
     - Сохраняет изменения в базе данных.

3. **Просмотр подписчиков и подписок:**
   - Когда пользователь запрашивает список подписчиков или подписок, система:
     - Находит пользователя по его ID.
     - Возвращает массив `followers` или `following`, заполненный данными о пользователях (имя, email, аватар).

---

### Пример использования

1. Пользователь Jane Doe подписывается на John Doe:
   ```bash
   POST /users/64f1a2b3c4d5e6f7a8b9c0d1/follow
   Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
   ```

2. Пользователь Jane Doe отписывается от John Doe:
   ```bash
   POST /users/64f1a2b3c4d5e6f7a8b9c0d1/unfollow
   Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
   ```

3. Пользователь John Doe просматривает своих подписчиков:
   ```bash
   GET /users/64f1a2b3c4d5e6f7a8b9c0d1/followers
   Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
   ```
...
### Зависимости
- Для работы системы подписок требуется:
  - Аутентификация пользователя (JWT токен).
  - Модель пользователя с полями `followers` и `following`.
